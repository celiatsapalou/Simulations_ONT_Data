rule all:
    input:
        expand("res/R_script_output/{prefix}_{length_start}-{length_end}_Delly_minimap2_plot.png", prefix=config["prefix"], length_start=config["length_start"], length_end=config["length_end"]),
        ...
        expand("res/R_script_output/{prefix}_{length_start}-{length_end}_sniffles_ngmlr_plot.png", prefix=config["prefix"], length_start=config["length_start"], length_end=config["length_end"]),
...
# simulate ONT reads of the mutated genome
rule sim_reads:
    input:
...
# Split the simulated reads by length for alignment
...
rule align_ref:
    ...
    shell:
        "minimap2 -a -x map-ont {input.ref_fa} {input.fasta} > {output.sam}"
...
rule sam_to_bam_minimap:
    input:
        sam = "res/align_ref_output/{prefix}_{length_start}-{length_end}_minimap_aligned.sam",
        ref = "data/ref_chr1.fa"  # Replace with the correct path to your reference file
    output:
        "res/align_ref_output/{prefix}_{length_start}-{length_end}_minimap.bam"
    shell:
        """
        samtools view -S -b {input.sam} | samtools calmd -b - {input.ref} > {output}
        """

#conversion sam to bam ngmlr
...
#run sniffles for minimap2 alignment
rule run_sniffles:
    input:
...
#run delly for minimap2 alignment
rule run_delly:
    input:
...
#run sniffles for ngmlr alignment
rule run_sniffles_ngmlr:
    input:
        bam="res/ngmlr_sniffles_ref_output/{prefix}_{length_start}-{length_end}_ngmlr_sorted.bam",
        index="res/ngmlr_sniffles_ref_output/{prefix}_{length_start}-{length_end}_ngmlr_sorted.bam.bai"
    output:
        "res/sniffles_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_variants_invs.vcf"
    shell:
        "sniffles -m {input.bam} -v {output}"

# Filter inversions specifically from Delly output
...
# Filter inversions specifically from Delly output for ngmlr
...
# Filter inversions specifically from sniffles output 
...
# Filter inversions specifically from sniffles output for ngmlr
rule filter_inv_sniffles_ngmlr:
    input:
      "res/sniffles_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_variants_invs.vcf"
    output:
      "res/sniffles_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_inv_sniffles_filtered.vcf"
    shell:
      "awk '($5 == \"<INV>\" || $1 ~ /^#/) {{print}}' {input} > {output}"

# Merge nearby variants from filtered Delly output
...
rule merge_nearby_variants:
    ...
    shell:
        ...
        """
# Merge nearby variants from filtered Delly ngmlr output
...
rule sniffles_to_bed:
    ...
    shell:
        ...
        """
#vcf to bed ngmlr sniffles
rule sniffles_to_bed_ngmlr:
    input:
        "res/sniffles_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_inv_sniffles_filtered.vcf"
    output:
        "res/sniffles_to_bed_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_sniffles.bed"
    shell:
        r"""
        awk -F '\t' '!/^#/ {{split($8,a,";"); for(i in a){{if(a[i] ~ /END=/){{split(a[i],b,"="); print $1 "\t" $2 "\t" b[2]}}}}}}' "{input}" > "{output}"
        """

# Format simulated BED file for consistency or further processing
rule sim_bed_format:
    input:
...
# Overlap analysis using BEDTools for Delly results
...
# Overlap analysis using BEDTools for Delly results with NGMLR alignments
...
# Overlap analysis using BEDTools for sniffles results with minimap2 alignments
...
# Overlap analysis using BEDTools for sniffles results with NGMLR alignments
rule bedtools_intersect_sniffles_ngmlr:
    input:
        "res/simulated_invs_bed/{prefix}_invs_ONT.bed",
        "res/sniffles_to_bed_output_ngmlr/{prefix}_{length_start}-{length_end}_ngmlr_sniffles.bed"
    output:
        "res/bedtools_intersect_sniffles_ngmlr/{prefix}_{length_start}-{length_end}_for_R.bed"
    envmodules: "BEDTools/2.30.0-GCC-11.2.0"
    shell:
        "bedtools intersect -a {input[0]} -b {input[1]} -f 0.7 -r -wao > {output}"

#create R plot
rule run_R_script_sniffles_ngmlr:
    input:
        "res/bedtools_intersect_sniffles_ngmlr/{prefix}_{length_start}-{length_end}_for_R.bed"
    output:
        "res/R_script_output/{prefix}_{length_start}-{length_end}_sniffles_ngmlr_plot.png"
...
rule run_R_script_delly:
    input:
        "res/bedtools_intersect_delly/{prefix}_{length_start}-{length_end}_for_delly.bed"
    output:
        "res/R_script_output/{prefix}_{length_start}-{length_end}_Delly_minimap2_plot.png"
    envmodules: "R/4.2.2-foss-2022b"
    shell:
        r"""
        /g/korbel/hoeps/anaconda_2022/miniconda3/envs/nahrwhals-big/bin/Rscript scripts/for_Delly.R {input} {output}
